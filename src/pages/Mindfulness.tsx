...
const EXERCISES: Exercise[] = [
  {
    ...
    icon: '📦',
    type: 'breathing',
    steps: ['Inhale slowly for 4 counts', 'Hold your breath for 4 counts', 'Exhale completely for 4 counts', 'Hold empty for 4 counts'],
  },
  {
    id: '478-breathing',
    ...
    icon: '🌬️',
    type: 'breathing',
    steps: ['Inhale through nose for 4 counts', 'Hold breath for 7 counts', 'Exhale through mouth for 8 counts', 'Repeat 4 cycles'],
  },
  {
    id: '5-4-3-2-1',
  ...
  },
  {
    ...
    icon: '🧘',
    type: 'grounding',
    steps: ['Start at your toes — tense for 5s, then release', 'Move to calves and shins — tense and release', 'Tighten your thighs and buttocks, then release', 'Tense your stomach — breathe in, hold, release', 'Shrug shoulders to ears — hold 5s, drop them', 'Clench jaw gently — release and let face soften'],
  },
  {
    id: 'visualization',
    ...
    icon: '🏔️',
    type: 'visualization',
    steps: ['Close your eyes and breathe naturally', 'Picture a place where you feel completely safe', 'Notice details — colors, sounds, temperature', 'Feel the calm of this place in your body', 'Stay here as long as you need', 'Slowly return, carrying this calm with you'],
  },
];
...
function BreathingCircle({ exercise, onComplete }: BreathingCircleProps) {
  ...
  const phases = isBox
    ? [{ label: 'Inhale', duration: 4 }, { label: 'Hold', duration: 4 }, { label: 'Exhale', duration: 4 }, { label: 'Hold', duration: 4 }]
    : is478
      ? [{ label: 'Inhale', duration: 4 }, { label: 'Hold', duration: 7 }, { label: 'Exhale', duration: 8 }]
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-6">
        ...
        <motion.div
          ...
          onAnimationComplete={() => {
            ...
            if (nextPhase === 0) {
              ...
              setCycles(nextCycle);
              if (nextCycle >= targetCycles) { onComplete(); return; }
            }
            setPhase(nextPhase);
          }}
        >
      ...
      <div className="text-center">
        <p className="text-white/50 text-sm">Cycle {cycles + 1} of {targetCycles}</p>
        <div className="flex gap-1 justify-center mt-2">
          {Array.from({ length: targetCycles }).map((_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full ${i < cycles ? 'bg-violet-400' : 'bg-white/15'}`} />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl mx-auto">
      <MotionWrapper>
        ...
        <p className="text-white/50 text-sm mt-1">Breathing and grounding exercises for exam stress</p>
      </MotionWrapper>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            {EXERCISES.map((ex, i) => (
              <MotionWrapper key={ex.id} delay={i * 0.06}>
                <Card className="cursor-pointer hover:border-violet-500/30" onClick={() => start(ex)}>
        ...
        {selected && running && selected.type === 'breathing' && (
          <motion.div key="breathing" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card glow>
        ...
        {selected && running && selected.type !== 'breathing' && (
          <motion.div key="grounding" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card glow>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-white font-semibold">{selected.icon} {selected.name}</h2>
                <Button variant="ghost" size="sm" onClick={reset}>✕</Button>
              </div>
              <AnimatePresence mode="wait">
                <motion.div
                ...
                >
                  <p className="text-4xl mb-4" aria-hidden>{stepIndex + 1}</p>
                  <p className="text-white/80 text-lg leading-relaxed">{selected.steps[stepIndex]}</p>
                </motion.div>
              </AnimatePresence>
              <div className="flex items-center gap-3 mt-4">
                <Button variant="ghost" size="sm" onClick={() => setStepIndex((i) => Math.max(0, i - 1))} disabled={stepIndex === 0}>←</Button>
                <div className="flex gap-1 flex-1 justify-center">
                  {selected.steps.map((_, i) => (
                    <div key={i} className={`h-1 rounded-full transition-all ${i === stepIndex ? 'w-6 bg-violet-400' : 'w-2 bg-white/15'}`} />
                  ))}
                </div>
                {stepIndex < selected.steps.length - 1
                  ? <Button variant="primary" size="sm" onClick={() => setStepIndex((i) => i + 1)}>Next →</Button>
                  : <Button variant="primary" size="sm" onClick={finish}>Complete ✓</Button>
                }
              </div>
            </Card>
          </motion.div>
        ...
        {completed && selected && (
          <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
            <Card className="text-center py-10">
              ...
              <p className="text-white/60 text-sm mb-6">You completed {selected.name}. Take a moment to notice how you feel.</p>
              <div className="flex gap-3 justify-center">
                <Button variant="primary" onClick={() => { setRunning(true); setCompleted(false); setStepIndex(0); }}>Repeat</Button>
