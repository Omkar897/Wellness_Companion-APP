    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Trend', value: stats.trend === 'improving' ? '↑ Improving' : stats.trend === 'worsening' ? '↓ Worsening' : '→ Stable' },
        ...
        <MotionWrapper delay={0.2}>
          <Card glow>
            <div className="flex items-center gap-2 mb-4">
              <span className="w-7 h-7 rounded-lg bg-violet-600/30 border border-violet-500/40 flex items-center justify-center text-sm" aria-hidden>🧠</span>
              <div>
                <h2 className="text-sm font-semibold text-white">Weekly AI Report</h2>
                <p className="text-xs text-white/40">{new Date(latestInsight.generatedAt).toLocaleDateString('en-IN')}</p>
              </div>
            </div>

            <p className="text-white/80 text-sm leading-relaxed mb-4">{latestInsight.progressNote}</p>

            {latestInsight.patterns.length > 0 && (
              <div className="mb-4">
                <h3 className="text-xs font-semibold text-violet-300 mb-2 uppercase tracking-wide">Patterns Found</h3>
            ...
            {latestInsight.recommendations.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-cyan-400 mb-2 uppercase tracking-wide">Recommendations</h3>
                <ul className="flex flex-col gap-1.5">
